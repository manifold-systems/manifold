...
Manifold's features.  But `manifold` already brings in a lot of Manifold including
...
### Java 8
Here is a sample `build.gradle` file using `manifold` with **Java 8**:
```groovy
plugins {
    id 'java'
}
...
dependencies {
    compile group: 'systems.manifold', name: 'manifold', version: '2019.1.10'
...
### Java 11+
Here is a sample `build.gradle` file using `manifold` with **Java 11**:
```groovy
plugins {
    id 'java'
}
...
dependencies {
    ...
    annotationProcessor group: 'systems.manifold', name: 'manifold', version: '2019.1.10'
}
