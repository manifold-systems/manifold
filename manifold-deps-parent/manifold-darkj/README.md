...
The `manifold-darkj` dependency works with all build tooling, including Maven and Gradle. It also works with Java versions 8 - 12.

Here are some sample build configurations references.

>Note you can replace the `manifold-darkj` dependency with **`manifold-all`** as a quick way to gain access to all of
Manifold's features.

## Gradle

### Java 8
Here is a sample `build.gradle` file using `manifold-darkj` with **Java 8**:
```groovy
plugins {
    id 'java'
}
...
dependencies {
    compile group: 'systems.manifold', name: 'manifold-darkj', version: '2019.1.10'
...
### Java 11+
Here is a sample `build.gradle` file using `manifold-darkj` with **Java 11**:
```groovy
plugins {
    id 'java'
}
...
dependencies {
    ...
    annotationProcessor group: 'systems.manifold', name: 'manifold-darkj', version: '2019.1.10'
}
