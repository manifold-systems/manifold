...
public class DelegationIssueMsg
{
  ...
  public static final IssueMsg MSG_INTERFACE_LINK_FIELD_TYPE_EXPECTED_1 = new IssueMsg( "Interface expected for link type, but found '{0}'. Consider changing it to '{1}' or by using '@link({2})'." );
  public static final IssueMsg MSG_INTERFACE_LINK_FIELD_TYPE_EXPECTED_N = new IssueMsg( "Link interfaces are unspecified for field `{0}', the interfaces will be computed as if using '@link({{1}})'" );
  public static final IssueMsg MSG_LINK_TYPE_MUST_MATCH_ABSTRACT_PART_TYPE = new IssueMsg( "Link type '{0}' must match abstract part type '{1}'" );
}