...
public class ScratchTest_SakilaTest extends PostgresDdlServerTest
{
  @Test
  public void testSomeInterestingQueries() throws IOException
  {
    loadData( "/samples/data/postgres-sakila-data.sql" );

    Stores s = "[Stores.sql:PostgresSakila/] Select * From store";
    for( Store r : s.fetch() )
    {
    ...
    */
    ...
    /* [CumulativeRevenueAllStores.sql:PostgresSakila/]
...
//    auto one = "[.sql:PostgresSakila/] select 1 from dual".fetchOne();
//    System.out.println(one.display());


    for( Staff staff: "[.sql:PostgresSakila/] select * from staff".fetch() )
    {
