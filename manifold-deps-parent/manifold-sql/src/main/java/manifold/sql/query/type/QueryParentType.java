 ...
 */
class QueryParentType extends SqlParentType
{
  QueryParentType( SqlModel model )
  {
    super( model );
  }
  ...
  {
    return (QueryTable)_model.getSqlStatement();
  }
                                ...
                                String methodName, @SuppressWarnings( "unused" ) String returnType )
  {
    ...
    String query = getQuery() == null ? "<errant query>" : getQuery().getSqlSource();
  ...
  private String getParameterInfo()
  {
    ...
    List<Parameter> parameters = getQuery().getParameters();
  ...
  private void addRequiredParameters( AbstractSrcMethod method )
  {
    ...
    for( Parameter param: getQuery().getParameters() )
    {
