 ...
 * Note if you intend to implement JSON coercions you should instead implement {@code IJsonFormatTypeResolver}.
 * <p>
 * See {@code IJsonFormatTypeResolver}
 */
public interface ICoercionProvider
{
  ...
  /**
   * Coerce {@code value} parameter to a value suitable for a {@link javax.script.Bindings} e.g., a JSON bindings.
   * It is imperative that this method coerces only between the JSON types and Java types it explicitly
   * handles otherwise you may inadvertently perform a coercion that is incorrect that is better handled
   * by another coercion provider.
   * @param value A value to coerce.
   * @return A value suitable for a {@link javax.script.Bindings} or {@link ICallHandler#UNHANDLED}
