...
>Note you can replace the `manifold-yaml` dependency with **`manifold-all`** as a quick way to gain access to all of
...
### Java 8
Here is a sample `build.gradle` file using `manifold-yaml` with **Java 8**:
```groovy
plugins {
    id 'java'
}
...
dependencies {
    compile group: 'systems.manifold', name: 'manifold-yaml', version: '2019.1.10'
...
```groovy
rootProject.name = 'MyYamlProject'
```

### Java 11+
Here is a sample `build.gradle` file using `manifold-yaml` with **Java 11**:
```groovy
plugins {
    id 'java'
}
...
dependencies {
    ...
    annotationProcessor group: 'systems.manifold', name: 'manifold-yaml', version: '2019.1.10'
}
