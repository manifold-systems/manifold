...
components, or open recursion. Forwarding-based constructs compose independent components, but a component's self-calls do not dispatch through the composite. Trait, mixin, and family-based composition preserves self-call dispatch but fuses
the components into the class or binds them to an enclosing family, giving up their
independence.

We present \emph{Parts}, a practical model for object composition with
independent components and open recursion, realized as a compiler plugin
for Java~8--26.
...
Static typing is what makes the two hard to combine (Section~\ref{sec:related}).
...
hazard this constraint prevents a formal, soundness-proven treatment, though they
decline the disciplines Parts adopts here as
...
Multi-root requires a part to carry multiple composite identities, one per interface. ISD resolves this by separating two
...
remaining interfaces, producing multi-root at the part level. The possession example in
Section~\ref{sec:model} demonstrates this directly: \code{Demon} claims
only \code{Actor} from \code{Wizard} (partial delegation), which by propagation
gives \code{Hero} separate roots for \code{Combatant} and
\code{Actor} (multi-root).
...
an explicit parameter; ISD keeps it implicit and per-interface, over independent
runtime components with static interface contracts, at the cost of a virtual call.
...
whose uses may extend beyond this paper. Problems currently addressed through dynamic dispatch, AOP weaving, or collapsed component hierarchies may have cleaner solutions within it. The extent of that solution space is an open question.
...
DelphJ~\cite{gerakios2013} is a research language in which a delegatee is supplied
at run time and its self-calls route through the wrapping composite, giving open
recursion over independent objects under static typing. Two practical costs
distinguish it from Parts. Forwarding is not automatic: a \code{subobject} ``does
not imply any automatic forwarding,'' so each composite requires manually written
scripts using a ``morphing'' tool to identify the methods it forwards. And dispatch
resolves over a runtime access path rather than a constant-time indexed slot. It is
also difficult to determine how far the model extends to arbitrary, multi-root compositions
(Section~\ref{subsec:identity}).
...
object, recovering open recursion across independent components at the cost of a
virtual call; the two properties are not inherently in conflict.
