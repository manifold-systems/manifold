...
Filling this gap while retaining the essential properties of ordinary object composition remains an unmet challenge.

We present Parts, a general object composition model that meets this challenge by unifying independent runtime components,
...
as a compiler plugin for Java 8--26 with IDE integration, and its dispatch cost is $O(1)$, empirically equivalent to a vtable
call.
\end{abstract}
...
object-oriented languages provide some level of support for object composition, offering an alternative to implementation inheritance and
its well-known hazards such as fragile base classes and coupling to superclass internals. But conventional object
composition gives up a vital property that makes inheritance effective: \emph{open recursion}. With open recursion, a component's
self-calls remain late-bound, allowing them to reach overrides supplied by delegating composites. Without it, self-calls
...
The challenge is to retain the properties that make ordinary object composition favorable while recovering open recursion.
In particular, a general composition model should preserve \emph{independent components} (late-bound objects supplied at
construction) and \emph{arbitrary composition} (unrestricted delegation topologies), while providing \emph{open recursion}
(self-calls through the composite). Existing composition models generally provide at most two of these properties (Section~\ref{sec:related}).
...
composition cannot freely define its structure at runtime.

Several research projects (Section~\ref{sec:related}) have explored delegation-based models that retain independent components
while restoring open recursion. These approaches, however, support only restricted composition topologies rather than the
arbitrary topologies of conventional object composition (Section~\ref{subsec:topologies}). Most introduce auxiliary mechanisms
for maintaining a level of composite identity. This mechanism tends to increase overall implementation complexity and, in
some cases, incurs substantial runtime overhead.

Parts addresses these limitations. A \emph{part} is any class annotated with \code{@part}; it is an independent component supplied
at construction. A \emph{composite} is a regular class with one or more fields annotated with \code{@link}; it delegates the interfaces
declared on its linked fields to parts or plain objects. At link-field assignment, Parts stores the composite identity for
each linked interface in the assigned part. Through this identity, a part's self-calls reach composite overrides while the part remains an independent
runtime object. Because a link accepts any implementation of its declared interface, ordinary objects can still be used
as forwarding components, allowing Parts to compose with types not designed for it. Parts therefore subsumes conventional
composition by allowing both simple forwarding and open-recursive delegation to coexist within the same composite.

The enabling mechanism is \emph{interface-scoped dispatch} (ISD). By assigning composite identity independently for each
interface, ISD permits open recursion across arbitrary composition topologies while preserving independent runtime components.
Because dispatch is interface-scoped, self-calls retain $O(1)$ dispatch cost.
...
part links only: it terminates at any non-part link, a regular object with
self-calls remaining local to the delegate. A composition may freely mix parts
...
\emph{Partial delegation} is a requirement for arbitrary compositions, where a composite may claim only some of a part's
...
without compromise. The result is a general model that retains the essential properties of ordinary object composition---independent
