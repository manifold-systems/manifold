...
\title{Interface-Scoped Dispatch:\texorpdfstring{\\}{ }General Delegation with \texorpdfstring{$\boldsymbol{O(1)}$}{O(1)} Self-Calls}
...
\begin{abstract}
The conceptual attraction of object composition is its ability to freely build loosely coupled systems from independent
runtime components. Yet practical limitations in today's languages prevent composition from fully realizing this potential,
chiefly the inability to perform late-bound self-calls (open recursion), a defining strength of implementation inheritance.
Filling this gap while retaining the essential properties of object composition remains an unmet challenge.

We present \emph{Parts}, a general delegation model that meets this challenge by unifying independent runtime components,
arbitrary composition, and open recursion. Its enabling mechanism, interface-scoped dispatch (ISD), assigns composite identity
per interface at construction, allowing self-calls to dynamically dispatch through overriding composites. Parts is realized
as a compiler plugin for Java 8--26 with IDE integration. Its dispatch cost is $O(1)$, empirically equivalent to a vtable
call.
\end{abstract}
...
Object composition is the standard prescription for structuring behavior from reusable components~\cite{gamma1994}. Most
object-oriented languages provide varying levels of composition through \emph{forwarding}~\cite{kniesel2000}, offering an alternative
to implementation inheritance and its well-known hazards such as fragile base classes and coupling to superclass internals.
But forwarding gives up a vital property that makes inheritance effective: \emph{open recursion}. With open recursion, a component's
self-calls remain late-bound, allowing them to reach overrides supplied by forwarding composites. Without it, self-calls
are fixed to the component's local implementation and never reach composite overrides. Yet modern languages provide no practical
way to retain this behavior.

The challenge is to retain the properties that make object composition favorable while recovering open recursion.
In particular, a resolution must preserve \emph{independent components} (late-bound objects supplied at
construction) and \emph{arbitrary composition} (unrestricted composition topologies), while providing \emph{open recursion}
(self-calls through the composite). Mainstream composition models generally provide at most two of these properties (Section~\ref{sec:related}).

The conventional object composition of modern languages differs in implementation but reduces to basic forwarding. Some,
like Rust and Swift, require manual forwarding; others, such as Kotlin's \code{by} clause~\cite{kotlin} and Go's struct
embedding~\cite{go}, provide it automatically. They all support independent components under arbitrary composition, but
forwarding does not preserve the composite identity needed for open recursion: a component's \code{this} remains the component,
so its self-calls cannot be redirected through the composite. This is the \emph{Object Schizophrenia} problem~\cite{sekharaiah2002,kniesel2000}.

Trait composition, including Scala's \code{with}~\cite{odersky2005} and mixins~\cite{bracha1990}, makes the opposite trade.
Self-calls through \code{this} dispatch in the composed hierarchy, so overrides remain visible. But traits are fused into
the hierarchy at compile time and execute as a single runtime object. Components lose independent runtime identity, and
composition is structurally static and cannot be defined at runtime.

Several research projects (Section~\ref{sec:related}) have explored \emph{delegation}~\cite{lieberman1986,kniesel1999} models
that retain independent components while restoring open recursion. So far, these models support only restricted composition
topologies rather than the arbitrary topologies of object composition (Section~\ref{subsec:topologies}). Most introduce
auxiliary mechanisms for maintaining a level of composite identity. These mechanisms tend to increase overall implementation
complexity and, in some cases, incur substantial runtime overhead.

As a general delegation model, Parts addresses these limitations by unifying independent runtime components,
arbitrary composition, and open recursion. Its enabling mechanism, interface-scoped dispatch (ISD), uses \emph{links} to assign
composite identity per interface at construction to \emph{parts}, components that participate in delegation. A part's self-calls
then dispatch dynamically through the composites that link it. Per-interface identity is what carries this across arbitrary
composition topologies while the parts remain independent runtime objects. Links work with both part and non-part components,
supporting delegation with parts and forwarding with non-parts, allowing both to coexist in the same composite.
Parts is realized as a compiler plugin for Java 8--26 with IDE integration; because dispatch is interface-scoped, its cost
is $O(1)$, empirically equivalent to a vtable call.

The core contributions are:

\begin{itemize}
  \item \textbf{Parts}: a general delegation model in which
...
\code{takeAction()} to the \code{Hero}, where the \code{attack()} call would stay on
...
part type is a compile error; interface or Object types are required (Section~\ref{subsec:safety}).
...
The following example shows a part with interfaces rooted at different
...
Parts is defined over a standard typed object model where classes implement interfaces and method dispatch is defined by
the receiver's runtime type.

\begin{description}
  \item[claim] an interface of a part is claimed when a composite links that interface, making the composite its exclusive
  identity within the part.

  \item[component] a class nominally implementing one or more interfaces, available for delegation if the class is a part
  and for forwarding otherwise.

  \item[composite] a class containing one or more links. A composite nominally implements the interfaces declared on its
  links.

  \item[delegation surface] a component's interface hierarchy, excluding any interface marked \code{@internal} in its \code{implements}
  clause.

  \item[interface] a stateless abstract type that defines a contract (a Java interface).

  \item[link] an ordinary field annotated with \code{@link} and typed as an interface. The enclosing class (the composite)
  delegates the link's declared interface to the component assigned to the link at construction. Links are implicitly \code{private}
  and \code{final}; explicit modifiers are redundant.

  \item[part] a component annotated with \code{@part}, which signals compiler plugin processing for open recursion support.

  \item[partial delegation] a requirement for arbitrary compositions, where a composite may claim only some of a part's
  delegation surface.

  \item[root (of an interface)] the linking composite claiming an interface of a part.
\end{description}

When a composite links an interface to a part, the part is directly wired to the composite; it records a \emph{composite
identity}: a reference to the composite statically indexed by the interface. The part's self-calls route through the composite
indexed by the interface of the called method. The enabling mechanism is \emph{interface-scoped dispatch} (ISD).

A part may itself contain link fields, in which case composite identity propagates transitively. When such a part is wired
to an outer composite, that composite becomes the dispatch root for each interface it claims throughout the delegation chain,
while interfaces it does not claim retain the roots established by inner composites. This \emph{transitive propagation}
traverses part links only; it terminates at any non-part link, where self-calls are fixed within the ordinary object. A
composition may freely mix parts and non-part delegates, with no interaction between them.

Composites must also resolve \emph{diamonds}. When a composite has multiple links that share a common interface, an authoritative
link must be chosen via \code{@link(share=I.class)} at one of the link sites. This determines the leg of the diamond the
linking composite forwards through. The other links wire the composite normally: self-calls on the shared interface always
dispatch through the linking composite to the component on the authoritative leg.

Two practical extensions, composite-scoped encapsulation (\code{@internal}) and abstract parts with deferred linkage (\code{asLink()}),
are defined and implemented (Section~\ref{sec:impl}).

\paragraph{Dispatch rule.}
Traditional object-oriented dispatch resolves a call through the receiver's object identity:
\[ \text{call } m() \text{ on } o \;\longrightarrow\; \mathit{vtable}(o,\, m) \]
Interface-scoped dispatch replaces the single object identity with a per-interface composite identity. A self-call $m()$
in part $P$ where $m \in I$ resolves through the composite identity for $I$:
\[ \text{self-call } m() \in I \text{ in } P \;\longrightarrow\; \$\mathit{selves}_P[I].m() \]
For self-calls within a part's methods, the dispatch triple is \emph{(part, interface, method)} rather than the conventional
\emph{(object, method)}.
Figure~\ref{fig:dispatch-comparison} contrasts this per-interface routing with a single shared composite identity. This is
\emph{open recursion}: a method's self-calls late-bind to the most-derived receiver. Here the receiver resolves from the
interface of the self-call. Forwarding is the closed case: a component's self-calls stay in the component, and the composite
never enters the recursion.
...
\begin{invariant}
\label{prop:dispatch}
For any part $P$ in a delegation chain for interface $I$ rooted at composite $C$: a self-call \code{this.m()} within $P$
where $m \in I$ dispatches to $C$.
\end{invariant}
...
Mirroring the structural flexibility of standard object composition and forwarding, Parts supports unrestricted topologies.
These are expressed as behaviorally acyclic graphs where nodes are parts, composites, or ordinary objects, and edges are
link fields. Each edge establishes a connection from a composite to a component through the interface specified by the link.

Partial delegation permits a composite to claim only the interfaces it needs from a part's delegation surface, leaving the
remaining interfaces for other composites to claim. This freedom gives rise to the four composition topology primitives
shown in Figure~\ref{fig:topologies} and classified over two dimensions: the number of roots a part maintains and the number
of sources the composition has. A root is an outermost node claiming one or more interfaces of a part's delegation surface;
a source is a node with no incoming link edges.
...
Ordinary forwarding already admits these topologies freely; nothing prevents the use of partial delegation to
...
directly before and after two phases: \emph{Enter} (symbol definition) and \emph{Analyze} (semantic processing).

For the Enter phase, the plugin generates structural scaffolding. Here, \code{@part} classes are processed to add
the \code{\$selves} identity array, the \code{\$linkPartToSelf} wiring method, and if the class is abstract, the \code{\$Impl}
subclass and \code{asLink()} factory methods. Composite classes are also processed here to add forwarding methods corresponding
with \code{@link} delegation. At Analyze, \code{@part} classes are processed to rewrite self-calls and to enforce self-preservation
rules (Section~\ref{subsec:safety}). \code{@link} field assignment is also rewritten here to invoke wiring, and to enforce \code{@internal}.

\subsection{Part Transformation}

Each part holds a \code{\$selves} array where the slots of the array each correspond with an interface on the part's delegation
surface. The ordering of the array's slots is compile-time-determined according to the part's class information.
...
Array slots are initialized with \code{this}, so a part's self-calls dispatch on itself by default.

Within a part, every call \code{this.m(args)} where \code{m} is declared in interface $I$ is rewritten to:

\begin{lstlisting}
((I) $selves[IDX_I]).m(args)
\end{lstlisting}

The cast ensures interface method invocation via the \code{invokeinterface} instruction, which guarantees a composite
receiver will itself route the call correctly within its runtime implementation, regardless of indirection such as with
bridge methods.

The ordinal \code{IDX\_I} is assigned per part over its own delegation surface, so it is local to that class: the rewritten
self-calls and the generated \code{\$linkPartToSelf} both reside in the part and share its indexing, and no index agreement
across separately-compiled classes is required.

\subsection{Composite Transformation}

A composite nominally implements every interface claimed by its links. For each method of a claimed interface that the composite
does not implement directly, the plugin generates a delegating method that forwards through the corresponding link field:

\begin{lstlisting}
R m(args) { return link.m(args); }
\end{lstlisting}

In composite classes, each link field assignment \code{this.link = part} is transformed to wire \code{part} at link assignment
via \code{part.\$linkPartToSelf(this, I.class)} where the composite (\code{this}) is assigned to the part's \code{\$selves}
slot corresponding with the link's declared interface $I$. Then, if the part itself links the same interface to another
part, it recursively calls \code{\$linkPartToSelf} to rewire the link's part to the calling composite.

\subsection{Abstract Part Support}

A part may be abstract, deferring some interface methods to the composite. Linking an abstract part obligates the composite
to implement those methods or to be declared abstract. When enforceable at compile time: the plugin generates a delegating
method for every interface method the part implements and none for a method it leaves abstract, so a concrete composite
is missing exactly those methods and \texttt{javac} rejects it unless they are supplied directly. This is the compositional
analog of the rule that a concrete class must implement its inherited abstract methods.

To make an abstract part instantiable for linking, the compiler generates a private \code{\$Impl} subclass with stubs that
throw \code{DelegationLinkageError}, and an \code{asLink()} factory per constructor:
...
Once wired, the stubs are unreachable for a statically configured abstract part: self-calls dispatch through \code{\$selves}
to the composite, where the compile-time obligation guarantees an implementation. The stubs remain to enforce the dynamically
configured case and the linking composite does not override a part's abstract method. The stubs also serve as a runtime
guard for an \code{asLink()} instance invoked before the part is wired.

\subsection{Default Methods}
\label{subsec:default}

If left unmitigated, a default method runs with the part as receiver, rather than the composite, when neither the composite
nor the part overrides it, resulting in incorrect self-call dispatch.

No bytecode instruction directly invokes a default with a receiver other than the current \code{this}, but a method handle
bound to an \code{invokedynamic} call site works around this limitation. When a part does not override a default, the plugin
generates an override that invokes the interface's default implementation in this way, with the receiver taken from the
interface's slot in \code{\$selves}, so the default's self-calls dispatch through that receiver. An explicit \code{I.super.m()}
in part code is rewritten the same way.
...
The site's bootstrap links it once to a fixed target (the default implementation), with the receiver as its only varying
argument. Being monomorphic, it is inlined by the JIT to the same type of array-indexed invocation as in Section~\ref{sec:eval}.

\subsection{Safety Guarantees}
\label{subsec:safety}

\paragraph{Cycle detection.}
If a part is wired to itself directly or transitively over the same interface, \code{De\-le\-ga\-tion\-Link\-age\-Er\-ror}
is thrown at assignment time during the recursive \code{\$linkPartToSelf()} call.

\paragraph{\code{@internal} enforcement.}
\code{@internal} may annotate an interface method or an interface type literal in a class's \code{implements} clause, with
distinct effects. A method annotated with \code{@internal} is accessible only within the composite, the same intent behind
\code{protected} in implementation inheritance, but more cleanly scoped for composition. A type literal in a class's \code{implements}
clause annotated with \code{@internal} removes that interface from the class's delegation surface: no composite may claim
it via a link field.

\paragraph{Self-preservation.}
Inside a part, \code{this} use is guarded. Field access, private calls, and callbacks through the part's own link fields
(the compositional analog of \code{super}) are all permitted. Passing or assigning \code{this} as the concrete part type
is not: that is a compile error, since a caller holding the bare part could invoke methods outside the composite dispatch
path and violate Invariant~\ref{prop:dispatch}. The only \code{this} that can leave is either interface-typed or
\code{Object}-typed: resolving to the composite claiming the interface or the part itself. A client receiving it reaches
...
dispatched through \code{invokedynamic}, carries no measurable cost over the
...
is not separately observable; it overlaps with the dispatch. The self-call
...
object graph the program constructs in any case. The cost is a bounded,
one-time increment to composite construction, amortized over every subsequent
self-call.

\subsection{Example: Teaching Assistant}

The following example exercises diamond dispatch resolution and self-call routing through the composite. A teaching assistant (TA)
is both a student and a teacher; both roles share a common \code{Person} base.
...
With forwarding, the shared \code{Person} methods
...
\emph{Partial delegation} is a requirement for arbitrary composition, where a composite may claim only some of a part's
delegation surface. \emph{Multi-rooted parts} result from this condition: a part's interfaces may be rooted at two or more
distinct identities, including the part itself and one or more composites, via any combination of the multi-rooted topologies
in Section~\ref{subsec:topologies}.

The possession example in Section~\ref{sec:model} demonstrates an extended version of topology (b): \code{Demon} claims
only \code{Actor} from \code{Wizard}. Through delegation propagation, \code{Hero} consequently has separate roots for \code{Actor}
and \code{Combatant}, making it a multi-rooted part through a single source graph, \code{Demon}. The PaidSubscriber/SuperSoldierAddon
example (Figure~\ref{fig:topology-c-listing}) demonstrates topology (c): multiple roots within a multi-source composition.

This multi-rootedness makes a part's identity context-sensitive. References to \code{this}, whether implicit or explicit,
resolve according to the context type of the reference. When that type is a claimed interface, ISD resolves \code{this}
to the claiming composite. Otherwise, \code{this} remains the part itself. This is a direct resolution of the Object Schizophrenia
problem mentioned in Section~\ref{sec:intro}.
...
\begin{itemize}
  \item \textbf{Fragile base class}~\cite{mikhajlov1998}: a composite's dependency surface is the interface contracts it
        explicitly claims, not a components's internal implementation; changes to its encapsulated internals do not propagate.
        Partial delegation completes the isolation: a composite is unaffected by changes to interfaces it did not claim.
  \item \textbf{Forced IS-A semantics}: reuse does not require a type relationship; the composite is a subtype of the interface,
        not of the component's implementation.
  \item \textbf{Hierarchy-bound}: implementation inheritance is rooted in a single runtime object and therefore supports
        only single-source topologies; a component cannot participate in multiple, independently rooted composites. Parts
        permits arbitrary topologies by assigning composite identity per interface.
  \item \textbf{The \emph{Object Schizophrenia} problem~\cite{sekharaiah2002}}: a hazard with forwarding
        where self-calls remain within the component rather than routing through the composite; resolved by ISD.
  \item \textbf{Diamond ambiguity}: explicit \code{@link(share=I.class)} resolution; unresolved diamonds are compile errors,
        not silent merges.
  \item \textbf{Encapsulation breach}: a composite may implement a subset of its parts' interfaces, those it does not implement
        are absent from its API by construction. \code{@internal} strengthens this further: a class may declare selected
        interfaces as internal to its implementation and excluded from its delegation surface, preventing the
        interfaces from being claimed by a composite.
  \item \textbf{Abstract behavior}: abstract parts with deferred linkage (\code{asLink()}) preserve the template-method
        capability of abstract classes without coupling their implementation and hierarchy.
\end{itemize}
...
member. Parts retains open recursion but bounds the surface to the declared interface contract; self-calls still reach the
composite's overrides, while the internals that make base classes fragile stay off it entirely.

Implementation inheritance and Parts are, essentially, two bindings of the same open recursion: implementation inheritance
binds it to the class hierarchy, where a subclass IS-A its superclass and inherits its entire implementation surface, while
Parts binds it to a held object and a declared interface, where the composite HAS-A its parts and IS-A only their interfaces.
The concept that inheritance and open recursion are separable is Cook's~\cite{cookpalsberg1989,cook1990}, and the modeling
of inheritance as composition is Bracha and Cook's~\cite{bracha1990}. Cook's model threads self as an explicit parameter;
Parts wires composite identity per-interface at construction.

\subsection{Concurrency}

A constructed composition is safe to share across threads. Each interface's \code{\$selves} slot is written only while its
final composite root (Invariant~\ref{prop:dispatch}) is under construction. Once that root is safely published the slot
is never written again, so the JMM makes all wired state visible to any thread observing the root, with no synchronization
on the dispatch path. The guarantee is scoped to a single composition, resting on the one-root invariant of Section~\ref{sec:discussion}.
Parts may hold mutable state under standard Java rules and dispatch adds no synchronization points.

\subsection{Future Work}
\label{sec:futurework}

\paragraph{Per-interface composite roots as a general primitive.}
Assigning dispatch identity per interface at construction is, perhaps, a primitive that may be applied in areas beyond this
paper's focus. Problems currently addressed through dynamic dispatch, AOP weaving, or collapsed component hierarchies may
have cleaner solutions within it. The extent of that solution space is an open question.

\paragraph{Formal verification.}
A mechanized proof of Invariant~\ref{prop:dispatch} in a small-step operational semantics would provide assurance beyond
the construction argument in Section~\ref{sec:model}. The model maps cleanly to a subset of Featherweight Java, which is
a natural starting point.

\paragraph{Link timing and mutability.}
Through immutable links, composite identity is finalized at construction. This read-only guarantee is what makes sharing
lock-free (Section~\ref{sec:discussion}): stable wiring needs no synchronization on the dispatch path. Two relaxations of
the rule, opt-in \emph{lazy} and \emph{mutable} links, are worth exploring, and they go in opposite directions.

A \emph{lazy} link changes only the timing. Backed by the stable-value facility of recent
JDK releases, it wires on first use instead of at construction, yet stays written once and
effectively final, so the guarantee holds while link initialization is deferred. A \emph{mutable} link
lifts the restriction. Mutating link assignments become legal, restoring the only aspect
of conventional forwarding Parts disallows---runtime re-configuration---but at the
price of the guarantee and under the same thread-safety obligations as any mutable field.
Both address restrictions of the current model, not limits of it. Whether a mutable link can
also keep the guarantee, by confining rewiring to safe-publication points, is an open question.
...
independent late-bound component. ISOLATE sits on the mixin side of the
...
composition~\cite[\S5.5.4]{kniesel2000}. DARWIN achieves open recursion,
...
confined to topology~(a) (\S\ref{subsec:topologies}), the single-rooted,
...
topologies~(b)--(d) that partial delegation induces with ordinary forwarding.
...
this space. A classless (prototype) language with a type system that separates
...
multi-source structure as topology (c) (\S\ref{subsec:topologies}): no subject is required to be the sole root
...
The advice to favor object composition over class inheritance came with an unspoken cost: composition gives up the open
recursion that made inheritance practical. Parts removes that cost. By assigning composite identity per interface at
...
without compromise. The result is a general model that retains the essential properties of simple forwarding---independent
runtime components and arbitrary composition topologies---while adding open recursion at vtable speed.
