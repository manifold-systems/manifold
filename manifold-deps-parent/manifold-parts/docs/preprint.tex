...
\begin{abstract}
Object composition under static typing forces a trade among three properties: independent components, open recursion, and
arbitrary composition. Ordinary object composition models compose independent runtime components through forwarding,
but a component's self-calls don't dispatch through the composite. Trait and mixin-based composition models preserve
self-call dispatch but fuse components into the class, resulting in a single runtime object, giving up independent components.
Other delegation-based composition models are forced to compromise by limiting component independence, restricting composition
structure, or incurring runtime overhead.

We present \emph{Parts}, a general composition model that removes the trade. Realized as a compiler plugin for Java 8–26,
Parts provides independent components, supplied as runtime objects at construction, with self-calls that route through the
composite in arbitrary compositions. The enabling mechanism, \emph{interface-scoped dispatch} (ISD), assigns composite identity
per interface at construction. The dispatch cost is $O(1)$, empirically equivalent to a vtable call.
\end{abstract}
...
Object composition is the standard prescription for structuring behavior from reusable components~\cite{gamma1994}. A general
composition model must reconcile three properties: \emph{independent components} (late-bound objects supplied at construction),
\emph{open recursion} (self-calls through the composite), and \emph{arbitrary composition} (unrestricted delegation topologies).
Existing composition models typically trade among these properties, particularly under static typing (Section~\ref{sec:related}).

Ordinary object composition via forwarding-based constructs (Kotlin \code{by}~\cite{kotlin}, Lombok
\code{@Delegate}~\cite{lombok}) supplies independent components under arbitrary composition, but does not
support open recursion because plain objects do not preserve references to the forwarding composites required to resolve
self-calls. This is the \emph{Object Schizophrenia} problem~\cite{sekharaiah2002}.
...
independent runtime objects at construction. Composition is static and compositional
structure is limited to a single-rooted hierarchy.

Family-based composition models recover open recursion while retaining component structure, but the family module controls
object construction, where members are instantiated within the family rather than supplied as independent objects: they
provide open recursion without independent components.

Delegation-based composition models resembling Parts provide independent components with open recursion, but fall short
of arbitrary composition without a dispatch mechanism that maps each of the composite's interface claims onto the corresponding
composite identity in the component. Such mechanisms restrict the delegation topologies that can be practically expressed
under arbitrary composition (Section~\ref{subsec:topologies}) and tend to introduce substantial runtime overhead (Section~\ref{sec:related}).

\emph{Parts} satisfies all three properties. A \code{@part} class is an independent object supplied
at construction; \code{@link} marks a field through which a composite delegates an interface to a part. When a
composite is constructed, the compiler plugin wires composite identity into each part per delegated interface, causing
self-calls in the part to dispatch through the composite. A link field accepts any implementation of its declared interface:
forwarding therefore works with arbitrary objects, while part objects additionally provide open recursion. Parts thus subsumes
forwarding rather than replacing it.
...
compositions. What the model offers is a general compositional foundation that
...
for arbitrary composition---by rooting identity in the interface rather than the object.
...
The limits inherent in the access path prevent the model from satisfying arbitrary composition:
multi-source topologies and behaviorally terminal cycles, for instance, are not expressible at all
with the access path strategy (Section~\ref{subsec:topologies}). Parts instead internalizes composite
identity in the component; its compact, self-contained design enables automated forwarding while preserving
$O(1)$ self-dispatch in arbitrary composition.
...
For decades, the conventional wisdom of object-oriented programming has favored composition over implementation inheritance,
yet mainstream languages have not delivered a general composition model that makes this practical.

Today's models still range between two failure modes: forwarding-based constructs that keep components independent but forgo
open recursion, and trait- or mixin-based models that recover open recursion by flattening composition back into the class
hierarchy, giving up the independent, dynamically composable objects that make composition worth choosing in the first place.

Models that push beyond inheritance while retaining open recursion instead resolve composite identity through a single \emph{self},
fixed at construction, or by reconstructing it per call. This limits such models to basic composition topologies, excluding
the advanced topologies of Section~\ref{subsec:topologies} and forfeiting arbitrary composition. These models also tend
to add substantial runtime overhead compared with the more mainstream models rooted in implementation inheritance.

Parts aims to provide a \emph{practical} general composition model. It combines independent runtime components, open recursion,
and arbitrary composition, while retaining $O(1)$ self-dispatch. By assigning composite identity per interface at construction,
Parts shows that there was always a clear path.
